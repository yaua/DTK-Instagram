    ...
    , m_height(nHeight)
{
    ...
    m_fullScreen->setDisabled(nFixSize);    //  Disable full screen mode when fixed window size
    ...
    /* The corresponding options are hidden in the GUI after setting the parameters on the command line */
    if(!nFixSize)
    {
...
void MainWindow::fullScreen()
{
    ...
    else
    {
        if(!mFixSize)
        {
            m_fixSize->setDisabled(false);  // When the command line parameter does not have a fixed window size, the fixed window size can be manually selected in window mode
        }
        m_menu->update();
        showNormal();
...
void MainWindow::fixSize()
{
    if(m_fixSize->isChecked())
    {
        ...
        /* There is a BUG: After enabling the tray icon, if you manually select a fixed window size and close the window, the maximize button will be lost when you open it again and cannot be restored*/
    }
    else
    {
...
{
    if(!m_fixSize->isChecked()) // Disable full screen when fixed window size
    {
        if(event->key() == Qt::Key_F11) // shortcut fullscreen with F11
        {
...
void MainWindow::on_downloadStart(QWebEngineDownloadItem *item)
{
    ...
    if(mutex.tryLock())
    {
        ...
        resume->hide();
        pause->show();
        this->message->show();  // The progress bar was hidden after the last download was completed, and it needs to be redisplayed here
    }
    else
    {
